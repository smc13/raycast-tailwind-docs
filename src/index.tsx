...
export default function DocList() {
  ...
  const client = algolia("BH4D9OD16A", "3df93446658cd9c4e314d4c02a052188");
  client.addAlgoliaAgent("Raycast Extension");
  const index = client.initIndex("tailwindcss");
  ...
  return (
    <List
      ...
      throttle
      navigationTitle="Search Tailwind Documentation"
...
async function fetchDocs(index: SearchIndex, query?: string | null): Promise<{ [key: string]: Document[] }> {
  try {
    const response = await index.search<AlgoliaDocument>(query ?? "", {
